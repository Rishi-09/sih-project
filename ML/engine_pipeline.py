"""
Pure NumPy / Python Engine Health Monitoring & Diagnostic Pipeline for R2 ML.

This module provides an end-to-end, ultra-fast inference engine for the R2 UAV Engine
Health Monitoring ML models:
1. Model 1: Nominal Digital Twin (19 sensor channels)
2. Model 2: Multiclass Fault Classifier (single / compound failure modes)
3. Model 3: Unsupervised Anomaly Detector (Isolation Forest)
4. Subsystem & Overall Health Index (0-100)
5. Prognosis / Remaining Useful Life (RUL)

Designed to run in standard Linux/Docker environments and Windows with zero
external C-extension DLL runtime dependencies, ensuring high portability and sub-20ms latency.
"""
...
def _predict_hgb_classifier(clf_model: Any, X_row: np.ndarray) -> Tuple[str, float, Dict[str, float]]:
    ...
    pred_class = str(clf_model.classes_[pred_idx])
    confidence = float(probs[pred_idx])

    prob_dict = {str(clf_model.classes_[i]): float(probs[i]) for i in range(n_classes)}
    return pred_class, confidence, prob_dict


def _eval_isolation_tree(tree: Any, x: np.ndarray) -> float:
    """Traverse a single Isolation Forest tree to get terminal path length."""
...
def estimate_rul(health_history: List[float], timestamps: Optional[np.ndarray] = None, failure_threshold: float = 20.0) -> Optional[float]:
    ...
    time_remaining = (failure_threshold - current_health) / slope
...
def _resolve_path(base_dir: Path, rel_path: str) -> Path:
    """Resolve a relative path checking base_dir, current working directory, and subfolder variants."""
    candidates = [
        base_dir / rel_path,
        Path.cwd() / rel_path,
        base_dir / rel_path.replace("ML/", ""),
        Path.cwd() / rel_path.replace("ML/", ""),
        base_dir / ("ML/" + rel_path),
    ]
    for p in candidates:
        if p.exists():
            return p
...
class EngineHealthPipeline:
    ...
    def __init__(
        self,
        nominal_twin_dir: str = "models/nominal_twin",
        fault_classifier_dir: str = "models/fault_classifier",
        anomaly_detector_dir: str = "models/anomaly_detector",
        healthy_stats_path: str = "data/processed/residuals/healthy_residual_stats.json",
        residual_domain: str = "training",
    ):
        base_dir = Path(__file__).resolve().parent
        twin_path = _resolve_path(base_dir, nominal_twin_dir)
        fc_path = _resolve_path(base_dir, fault_classifier_dir)
        ad_path = _resolve_path(base_dir, anomaly_detector_dir)
        stats_file = _resolve_path(base_dir, healthy_stats_path)

        # 1. Load Nominal Twin Bundle
        bundle_file = twin_path / "nominal_twin_bundle.joblib"
        if not bundle_file.exists():
            raise FileNotFoundError(f"Missing nominal twin bundle at {bundle_file}")
        ...
        if not fc_file.exists():
            raise FileNotFoundError(f"Missing fault classifier model at {fc_file}")
        ...
        if not ad_file.exists():
            raise FileNotFoundError(f"Missing anomaly detector model at {ad_file}")
        with open(ad_file, "rb") as f:
            self.anomaly_detector = joblib.load(f)

        # 4. Load healthy baseline stats
        if not stats_file.exists():
            raise FileNotFoundError(f"Missing healthy residual stats at {stats_file}")

        with open(stats_file, "r", encoding="utf-8") as f:
            sdata = json.load(f)
            self.healthy_stats = sdata.get("sensor_stats", sdata)

        # Phase-conditioned domain stats
        self.phase_stats = {}
    ...
    def _stats_for(self, sensor: str, phase: Optional[str]) -> Tuple[float, float]:
        """Calibrated (mean, std) for a channel, preferring phase-specific calibration if present."""
        if phase:
    ...
    def predict(
        ...
        for c in CONTEXT_COLS:
            if c not in telemetry_window.columns:
                raise ValueError(f"Missing required context column: {c}")
        for s in SENSOR_COLS:
            if s not in telemetry_window.columns:
                raise ValueError(f"Missing required sensor column: {s}")
...
_GLOBAL_PIPELINE: Optional[EngineHealthPipeline] = None

def get_pipeline() -> EngineHealthPipeline:
    global _GLOBAL_PIPELINE
    if _GLOBAL_PIPELINE is None:
        _GLOBAL_PIPELINE = EngineHealthPipeline()
