import { sensorContract, SensorSpec, resolveLimits, SENSOR_FAULT_BY_ID } from "./contract";
import { mulberry32 } from "./rng";
import {
  TickFrame, MissionBlock, MissionProfile, ReliabilityLimiter, WhatIfResult,
} from "../types";
 ...
 */
export const PENDING_MISSION: MissionBlock = {
  pSuccess: null,
  pSuccessLo: null,
  pSuccessHi: null,
...
interface MonteCarloResult {
  ...
  /** Quantiles of time-to-first-limit-breach, null when censored at the horizon. */
  rulP10: number | null;
  rulP50: number | null;
  rulP90: number | null;
}
...
interface Sample {
  t: number;
  throttle: number;
  throttleSlow: number; // EWMA of throttle — the thermal-lag regressor
  values: Record<string, number>;
}

/** A channel's fitted trend plus the limit it is heading for. */
interface ChannelModel {
  spec: SensorSpec;
  value: number; // current (fit-smoothed) value
  b0: number;
  b1: number; // drift per second, power-held-constant, post-shrinkage
  gain: number; // steady-state units per % throttle (b2 + b3 combined)
  se: number; // standard error of b1
  levelSe: number; // standard error of the fitted CURRENT LEVEL — see fitAll()
  uMin: number; // throttle range the gain was measured across — the only range
  uMax: number; // it may be applied over, see THROTTLE_EXTRAPOLATION_MARGIN_PCT
  uNow: number; // throttle at the anchor sample — see crossingTime()
  limitHigh?: number;
  limitLow?: number;
  cautionHigh?: number;
  cautionLow?: number;
...
/** One projected leg of the remaining mission: absolute times + power. */
...
export class ReliabilityEngine {
  ...
  private cache: { t: number; block: MissionBlock } | null = null;
  private throttleSlow: number | null = null;
  ...
  ingest(frame: TickFrame) {
    ...
    if (this.history.length > WINDOW_SEC) this.history.shift();
    this.cache = null;
  }
   ...
   */
  private inTransient(): boolean {
    if (this.history.length < 2) return true;
    let lo = Infinity;
    let hi = -Infinity;
    for (const s of this.history) {
      if (s.throttle < lo) lo = s.throttle;
      if (s.throttle > hi) hi = s.throttle;
    }
   ...
   */
  evaluate(frame: TickFrame): MissionBlock {
    if (this.cache && this.cache.t === frame.t) return this.cache.block;
   ...
   */
  whatIf(frame: TickFrame, powerPct: number): WhatIfResult {
  ...
  private compute(frame: TickFrame): MissionBlock {
    const remainingSec = this.remainingSec(frame.t);
    ...
    const binding =
      limiters.find((l) => l.secondsToLimit !== null) ??
  ...
  private remainingSec(t: number): number {
   ...
   */
  private projectLegs(nowT: number, horizonSec: number, capPowerPct: number | null): ProjectedLeg[] {
  ...
  private fitAll(frame: TickFrame): ChannelModel[] {
    if (this.history.length < MIN_SAMPLES) return [];
    ...
    let uMax = -Infinity;
    for (const s of this.history) {
      if (s.throttle < uMin) uMin = s.throttle;
      if (s.throttle > uMax) uMax = s.throttle;
    }
    ...
    for (const spec of sensorContract.engine) {
      ...
      if (limits.max === undefined && limits.min === undefined) continue;
   ...
   */
  private cautionHazard(m: ChannelModel): number {
   ...
   */
  private cappedSigma(sigma: number, spec: SensorSpec): number {
    const lo = spec.normalMin;
    const hi = spec.normalMax;
    if (lo === undefined || hi === undefined || hi <= lo) return sigma;
   ...
   */
  private faultProgressionHazard(frame: TickFrame): number {
    const label = frame.diagnosis.label;
    if (!label || label === "healthy") return 0;
    ...
    const hasEngineFault = label.split("+").some((part) => part && !SENSOR_FAULT_BY_ID.has(part));
    if (!hasEngineFault) return 0;
  ...
  private cautionDepth(m: ChannelModel): number {
  ...
  private fitChannel(
    channel: string,
  ): { value: number; b0: number; b1: number; gain: number; se: number; sigma: number } | null {
    const pts: { x: number[]; y: number }[] = [];
    for (const s of this.history) {
      const y = s.values[channel];
      if (typeof y === "number" && Number.isFinite(y)) pts.push({ x: [s.t, s.throttle, s.throttleSlow], y });
    }
    const n = pts.length;
    ...
    let my = 0;
    for (const p of pts) {
      for (let j = 0; j < k; j++) mx[j] += p.x[j];
      my += p.y;
    }
    for (let j = 0; j < k; j++) mx[j] /= n;
    my /= n;
    ...
    for (const p of pts) {
      ...
      for (let i = 0; i < k; i++) {
        c[i] += d[i] * dy;
        for (let j = 0; j < k; j++) M[i][j] += d[i] * d[j];
      }
    }
    if (M[0][0] <= 0) return null; // time never varies — no window at all
    ...
    let p = k;
    let solved: { b: number[]; inv00: number } | null = null;
    while (p >= 1) {
  ...
  private crossingTime(
    ...
    for (const leg of legs) {
      ...
      const at = (T: number) => base + slope * (Math.min(T, slopeEndT) - nowT);
      ...
      if (m.limitHigh !== undefined) {
        if (vStart >= m.limitHigh) return Math.max(0, leg.startT - nowT);
        if (vEnd >= m.limitHigh && slope > 0) {
          const tc = nowT + (m.limitHigh - base) / slope;
          if (tc >= leg.startT && tc <= Math.min(leg.endT, slopeEndT)) return Math.max(0, tc - nowT);
        }
      }
      if (m.limitLow !== undefined && leg.throttle >= LOW_LIMIT_MIN_POWER_PCT) {
        if (vStart <= m.limitLow) return Math.max(0, leg.startT - nowT);
  ...
  private monteCarlo(
    ...
    for (let trial = 0; trial < MC_TRIALS; trial++) {
      ...
      for (const m of models) {
        ...
        if (tc !== null && tc < earliest) earliest = tc;
      }
      ...
      if (earliest === Infinity) {
        ...
        if (earliest >= missionRemainingSec) survived++;
      }
    }
    ...
    for (const m of models) hazard += this.cautionHazard(m);
    ...
    const q = (frac: number): number | null => {
   ...
   * failing.
   */
  prognosis(frame: TickFrame): { rulSec: number | null; rulLoSec: number | null; rulHiSec: number | null; basis: string } {
    const models = this.fitAll(frame);
    if (models.length === 0) return { rulSec: null, rulLoSec: null, rulHiSec: null, basis: "insufficient_history" };
    const legs = this.projectLegs(frame.t, ENDURANCE_HORIZON_SEC, null);
   ...
   */
  private buildLimiters(models: ChannelModel[], legs: ProjectedLeg[], nowT: number): ReliabilityLimiter[] {
    const out: ReliabilityLimiter[] = [];
    for (const m of models) {
   ...
   */
  private headroomReference(m: ChannelModel, high: boolean): number {
    if (high) return m.spec.normalMax ?? m.cautionHigh ?? m.limitHigh! * 0.8;
  ...
  private decide(args: {
    pSuccess: number;
    airborne: boolean;
    binding: ReliabilityLimiter | null;
    alreadyBreached: ReliabilityLimiter | null;
    criticalAlert: boolean;
    remainingSec: number;
    safeEnduranceSec: number;
    frame: TickFrame;
    models: ChannelModel[];
  }): { recommendation: MissionBlock["recommendation"]; derateTo: number | null; reason: string } {
    ...
    if (pSuccess < P_CONTINUE) {
      ...
      const legsFor = (p: number) => this.projectLegs(args.frame.t, ENDURANCE_HORIZON_SEC, p);
   ...
   */
  private deratedModels(models: ChannelModel[], powerPct: number, currentThrottle: number): ChannelModel[] {
   ...
   */
  private confidenceRating(models: ChannelModel[]): "low" | "medium" | "high" {
    if (this.history.length < WINDOW_SEC * 0.6) return "low";
    ...
    if (resolved >= 1 || flat >= models.length * 0.8) return "high";
    return "medium";
  }
 ...
 */
function solveSymmetric(M: number[][], c: number[], p: number): { b: number[]; inv00: number } | null {
  let scale = 0;
  for (let i = 0; i < p; i++) scale = Math.max(scale, Math.abs(M[i][i]));
  ...
  for (let col = 0; col < p; col++) {
    ...
    const pv = a[col][col];
    for (let j = col; j < p + 2; j++) a[col][j] /= pv;
    for (let r = 0; r < p; r++) {
      if (r === col) continue;
      const f = a[r][col];
      if (f === 0) continue;
      for (let j = col; j < p + 2; j++) a[r][j] -= f * a[col][j];
    }
  }

  const b = new Array(3).fill(0);
  for (let i = 0; i < p; i++) b[i] = a[i][p];
  const inv00 = a[0][p + 1];
  if (!Number.isFinite(inv00) || inv00 <= 0) return null;
  return { b, inv00 };
}
 ...
 */
function sensorFaultChannels(frame: TickFrame): Set<string> {
  const out = new Set<string>();
  if (frame.diagnosis.sensorFault.channel) out.add(frame.diagnosis.sensorFault.channel);
  for (const part of frame.diagnosis.label.split("+")) {
    const spec = SENSOR_FAULT_BY_ID.get(part);
    if (spec) out.add(spec.channel);
  }
  return out;
}

function bindingReason(binding: ReliabilityLimiter | null, head: string): string {
  if (!binding) return head;
...
export function prettyChannel(id: string): string {
