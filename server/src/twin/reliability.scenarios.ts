...
function run(label: string, fault: { type: string; severity: number; at: number } | null) {
  const twin = new TwinRun("r1", "e1", 42, "test");
  const alerts = new AlertEngine();
  const rel = new ReliabilityEngine();
  console.log(`\n===== ${label} =====`);
  for (let i = 0; i < 900; i++) {
    if (fault && i === fault.at) twin.injectFault({ type: fault.type, severity: fault.severity });
    const f = twin.step();
    ...
    if (i % 120 === 0 || i === 899) {
      ...
      console.log(
        `t=${String(f.t).padStart(4)} ${f.phase.padEnd(8)} ehi=${(f.health.ehi === null ? "--" : f.health.ehi.toFixed(0)).padStart(3)}`,
        `P=${n2(m.pSuccess)}[${n2(m.pSuccessLo)}-${n2(m.pSuccessHi)}]`,
        `safe=${String(m.safeEnduranceSec ?? "--").padStart(4)}s rem=${m.missionRemainingSec}s`,
        `${m.recommendation}${m.derateTo ? `(${m.derateTo}%)` : ""}`,
        `conf=${m.confidence}`,
        b
          ? `| bind ${b.channel} ${b.value}${b.unit}->${b.limit} ttl=${b.secondsToLimit} rate=${b.ratePerMin}/min head=${b.headroomPct}%`
          : "| no limiter",
      );
      if (i === 480 || i === 899) console.log(`      reason: ${m.reason}`);
    }
  }
}
