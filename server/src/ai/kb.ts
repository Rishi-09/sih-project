/**
 * Structured knowledge-base lookup keyed by classifier fault id — not a vector
 * DB, not embeddings. Each entry doubles as (a) grounding context handed to the
 * LLM and (b) the offline fallback advisory served when no API key is set or
 * the call fails, so the AI panel never shows an error on stage. See published
 * plan §A7.
 */
export interface KbEntry {
  ...
  label: string;
  description: string;
  fallbackAdvisory: string; // markdown
}
